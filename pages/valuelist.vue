...
<div class="flex flex-col">
    <flicking v-show="show" class="p-4">
      ...
      <div class="flex-col w-full p-4 obstacle">
        ...
        <div class="flex flex-wrap justify-around p-3 ou-morphism">
            <ul class="w-[28rem] p-3 totu-morphism rounded-xl">
                <li v-for="(name, index) in valueName1" :key="'li1'+index" @mouseover="mouseOverAction(name)" @mouseleave="mouseLeaveAction" class="flex space-x-2 ml-3">
                    {{ list1[name] }}
                    <div v-show="hoverFlag && name === hoverName" class="ml-2">
                        <input type="radio" :id="'li1'+index" :name="name" @click="doubleCircle(index, 'li1',name)" />
                        <input type="radio" :id="'li1'+index" :name="name" @click="circle(index, 'li1',name)" />
                        <input type="radio" :id="'li1'+index" :name="name" @click="cross(index, 'li1',name)" />
                    </div>
                    <div :ref="setItemRef1"></div>
                </li>
            </ul>
            <ul class="w-[28rem] p-3 totu-morphism rounded-xl">
                <li v-for="(name, index) in valueName2" :key="'li2'+index" @mouseover="mouseOverAction(name)" @mouseleave="mouseLeaveAction" class="flex space-x-2 ml-3">
                {{ list2[name] }}
                <div v-show="hoverFlag && name === hoverName" class="ml-2">
                    <input type="radio" :id="'li2'+index" :name="name" @click="doubleCircle(index, 'li2',name)" />
                    <input type="radio" :id="'li2'+index" :name="name" @click="circle(index, 'li2',name)" />
                    <input type="radio" :id="'li2'+index" :name="name" @click="cross(index, 'li2',name)" />
                  </div>
                  <div :ref="setItemRef2">
                  </div>
                </li>
            ...
            <ul class="w-[28rem] p-3 totu-morphism rounded-xl">
                <li v-for="(name, index) in valueName3" :key="'li3'+index" @mouseover="mouseOverAction(name)" @mouseleave="mouseLeaveAction" class="flex space-x-2 ml-3">
                {{ list3[name] }}
                <div v-show="hoverFlag && name === hoverName" class="ml-2">
                    <input type="radio" :id="'li3'+index" :name="name" @click="doubleCircle(index, 'li3',name)" />
                    <input type="radio" :id="'li3'+index" :name="name" @click="circle(index, 'li3',name)" />
                    <input type="radio" :id="'li3'+index" :name="name" @click="cross(index, 'li3',name)" />
                  </div>
                  <div :ref="setItemRef3">
                  </div>
                </li>
    ...
    <div class="flex flex-raw  mb-5 p-4 ou-morphism">
        <div class="w-1/2 totu-morphism">
            <div class="text-center text-2xl">価値観リスト</div>
            <draggable class="dragArea list-group w-full" :list="ranks">
                <div class="list-group-item bg-gray-300 m-1 p-3 rounded-md text-center " v-for="(rank, index) in ranks" :key="rank">
                    <div @click="changeConcept(index)">{{ ranks[index] }}</div>
                    <teleport to="#concept">
                      <keep-alive>
...
import NewConcept from "~~/components/subject/myself/newConcept.vue";
    ...
    setup() {
        ...
        const doubleCircle = (index, listNumber, name) => {
            if (listNumber === 'li1') {
                ...
                if (!ranks.value.includes(list1[name]) && ranks.value.length <= 10)
                    ranks.value.push(list1[name]);
                }
            else if (listNumber === 'li2') {
                itemRefs2.value[index].innerHTML = "◎"
                if (!ranks.value.includes(list2[name]) && ranks.value.length <= 10)
                    ranks.value.push(list2[name]);
            }
            else {
                itemRefs3.value[index].innerHTML = "◎"
                if (!ranks.value.includes(list3[name]) && ranks.value.length <= 10)
